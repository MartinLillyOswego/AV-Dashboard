...
class Sender(threading.Thread):
    ...
    def package_data(self):
        ...
        print(f"{emulator_config.get_time()}:SendingThread: Sending: {last_packet}]")
    ...
    def send(connection, data):
        ...
        try:
            ...
            #print(f"{emulator_config.get_time()}:SendingThread: Sent: {data}")
        except serial.SerialTimeoutException:
            print(f"{emulator_config.get_time()}:SendingThread: Failed to send packet, bad serial connection")
            return False
        return True

    # main loop
    def run(self):
        print(f"{emulator_config.get_time()}:SendingThread: Started")
        connection = self.receiver.serial_port
        while True:
            starting_time = time.time()
            if not Sender.send(connection, self.package_data()):
                connection = self.receiver.serial_port
            time.sleep(emulator_config.SEND_INTERVAL-(time.time()-starting_time))
